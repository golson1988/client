...
type
  TfrmViewOtherNewItem = class(TForm)
...
var
  frmViewOtherNewItem: TfrmViewOtherNewItem;
...
procedure TfrmViewOtherNewItem.CloseAllDlg;
begin
  DJewelryBox.Visible := False;
  DZodiacSigns.Visible := False;
end;

procedure TfrmViewOtherNewItem.DJBCloseClick(Sender: TObject; X, Y: Integer);
begin
  DJewelryBox.Visible := False;
end;

procedure TfrmViewOtherNewItem.DZodiacCloseClick(Sender: TObject; X, Y: Integer);
begin
  DZodiacSigns.Visible := False;
end;

procedure TfrmViewOtherNewItem.Initialize;
var
  i : Integer;
begin
  DJewelryBox.DParent := FrmDlg.DBackground;
  DZodiacSigns.DParent := FrmDlg.DBackground;
  DZodiacSigns.DParent.DControls.Add(DZodiacSigns);
  DJewelryBox.DParent.DControls.Add(DJewelryBox);
  ...
  begin
    ...
    begin
      ...
      begin
        TDButton(Self.Components[i]).OnMouseMove := FrmDlg.DWeaponUS1MouseMove;
        //TDButton(Self.Components[i]).OnClick := FrmDlg.DSWWeaponClick;
        TDButton(Self.Components[i]).OnDirectPaint := FrmDlg.DNecklaceUS1DirectPaint;
