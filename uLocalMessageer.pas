...
implementation
  uses Wil, SoundUtil, MapUnit, MShare,EDcode,AnsiHUtil32;
...
begin
  ...
  FSocket.Addr := '127.0.0.128';//
  FSocket.OnDataAvailable := OnDataAvailable;
  Wil.LibManager.OnAddDownloadImage := AddDownloadImage;
  g_SoundManager.OnAddDownloadSoundFile := AddDownLoadSoundFile;
  g_SoundManager.OnAddDownloadFile := AddDownloadFile;
...
begin
  ...
  begin
    if Pos('91',AFileName) > 0 then
      g_SoundManager.LoadSkillSoundEffectList(AFileName)
    else
      g_SoundManager.LoadSoundList(AFileName);
  end;
