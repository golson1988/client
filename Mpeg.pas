...
uses
  Windows, DShow, ActiveX, Controls, uTypes;
...
type
  ...
  public
    ...
    function Play(const sFileName: WideString): Boolean;
    procedure Pause();
    procedure Stop();
    { Public declarations }
  end;
...
function TMPEG.Play(const sFileName: WideString): Boolean;
var
  _hr: Hresult;
begin
  Result := False;
  boInit := Init();
  _hr := g_pGraphBuilder.RenderFile(PWideChar(sFileName), nil);
  if failed(_hr) then
    exit;
  ...
  begin
    ...
    g_pVideoWindow.SetWindowposition(0, 0, MovieWindow.Width,
      MovieWindow.Height); // 播放的圖像爲整個panel1的ClientRect//
  end;
  g_pMediaControl.run;
  boPlay := true;
end;
